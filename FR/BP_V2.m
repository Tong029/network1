...
k=1;
kk(1,1)=1; 
%recognize which original experimental data set does each data come from and divide them into groups according to this.
%comparing the simulation results to each experimental data set
%a R2 and a RMSE would be calculated out for each experimental data set
for i = 1:sizev-1
    if validation.p(1,i+1)>validation.p(1,i)% when obtaining the data, only the data larger than its previous one would be chosen
        % so, if the data is smaller than its previous one, it means that
        % it belongs to a new experimental data set. 

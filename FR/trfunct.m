function [MEANrmse,MEANR2] = trfunct(extern)
%     clc
%     clear all
%     close all
    % load data
...
%     net.layers{3}.transferFcn = 'purelin';
    ...
    save('net');
    load('net');
copyfile('net.mat',strcat('net',num2str(extern),'.mat'));

%load the data without 0 values
...
%     save ERRORLoad.mat;
    ...
    B = validation.p(2,:);
    C = validation.t(1,:);
    SimRMSELoad = sim(net,[A;B]);%simulation result with validation set
    MEANrmse= sqrt(mean((SimRMSELoad-C).^2)); %RMSE
        %R2
        x = SimRMSELoad;% simulation
        y = validation.t(1,:);% experimental
        x_mean = mean(x);
        y_mean = mean(y);
        xy_mean = mean(x.*y);
        xx_mean = mean(x.*x);

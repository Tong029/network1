...
hold on; 
% simulation with rates range from 0 to 600 and with extension from 0 to 200;
tstrate = repmat(0:12:600,201,1).';
tstrate = tstrate.';
tstrate = tstrate(1:end);% save all the data as an array
tstex = repmat(0:200,51,1);
tstex = tstex.';
tstex = tstex(1:end);% save all the data as an array
predicting = sim(net,[tstex;tstrate]); % simulation 
